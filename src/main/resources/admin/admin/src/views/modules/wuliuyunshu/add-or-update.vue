	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px 0","background":"url(http://codegen.caihongy.cn/20230328/e3b3ef59d7a44ae6afcff886f7a76b98.jpg) no-repeat center bottom / 100% 100%","height":"auto"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="下单日期" prop="xiadanriqi">
					<el-input v-model="ruleForm.xiadanriqi" placeholder="下单日期" clearable  :readonly="ro.xiadanriqi"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="下单日期" prop="xiadanriqi">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="物流追踪" prop="wuliuzhuizong">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="物流追踪"
					  v-model="ruleForm.wuliuzhuizong" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.wuliuzhuizong" label="物流追踪" prop="wuliuzhuizong">
					<span :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.wuliuzhuizong}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='wuliuzhuizong'){
							this.ruleForm.wuliuzhuizong = obj[o];
							this.ro.wuliuzhuizong = true;
							continue;
						}
						if(o=='gengxinshijian'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wuliuyunshu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wuliuyunshuCrossAddOrUpdateFlag = false;
						 this.parent.search();
