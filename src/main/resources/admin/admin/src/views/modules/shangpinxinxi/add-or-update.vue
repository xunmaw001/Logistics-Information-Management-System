	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px 0","background":"url(http://codegen.caihongy.cn/20230328/e3b3ef59d7a44ae6afcff886f7a76b98.jpg) no-repeat center bottom / 100% 100%","height":"auto"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'" label="仓库编号" prop="cangkubianhao">
					<el-select :disabled="ro.cangkubianhao" @change="cangkubianhaoChange" v-model="ruleForm.cangkubianhao" placeholder="请选择仓库编号">
						<el-option
							v-for="(item,index) in cangkubianhaoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.cangkubianhao" label="仓库编号" prop="cangkubianhao">
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.$http({
				url: `option/kuweixinxi/cangkubianhao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.cangkubianhaoOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
			...
			cangkubianhaoChange () {
				this.$http({
					url: `follow/kuweixinxi/cangkubianhao?columnValue=`+ this.ruleForm.cangkubianhao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.cangkuweizhi){
							this.ruleForm.cangkuweizhi = data.data.cangkuweizhi
						}
					} else {
						this.$message.error(data.msg);
					}
